...
- **单机部署**: 使用 BadgerDB 作为元数据存储，配置简单且性能优秀
- **集群部署**: 使用 TiKV（基于 RocksDB + Raft）作为元数据存储，提供高可用性和可扩展性
...
1. **调整分块参数**: 根据数据特点调整 fastcdc 的分块参数以获得最佳去重效果
   ...
   - 配置文件位置: `config.properties` 中的相关参数
...
3. **去重缓存优化**: 适当调整去重缓存大小，平衡内存占用和去重效率
   ...
   - 配置文件位置: `config.properties` 中的 `cache.redis` 相关参数
...
2. **并发参数调整**: 根据硬件资源和负载情况调整并发参数
   ...
   - 配置文件位置: `config.properties` 中的服务器并发相关参数
