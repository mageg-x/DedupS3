...
// HTTPTarget 实现AuditTarget接口的HTTP版本
...
// NewHTTPTarget 创建一个HTTP审计目标实例
func NewHTTPTarget(serviceURL string, authToken string) (*HTTPTarget, error) {
	if serviceURL == "" {
		return nil, fmt.Errorf("http audit target requires service URL")
	}
...
// Send 通过HTTP发送单条审计日志
func (h *HTTPTarget) Send(ctx context.Context, entry *audit.Entry) error {
	// 将entry转换为JSON
	...
	// 创建请求
	req, err := http.NewRequestWithContext(ctx, "POST", h.serviceURL+"/api/audit/logs", bytes.NewBuffer(data))
	if err != nil {
	...
	if err != nil {
		logger.GetLogger("dedups3").Errorf("Failed to send audit log via HTTP: %v", err)
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusBadRequest {
		return fmt.Errorf("audit service returned non-success status: %d", resp.StatusCode)
	}
...
func (h *HTTPTarget) Query(ctx context.Context, cond *QueryCondition, opts *QueryOption) (*QueryResult, error) {
	...
	// 创建请求
	req, err := http.NewRequestWithContext(ctx, "POST", h.serviceURL+"/api/audit/query", bytes.NewBuffer(paramsData))
	if err != nil {
