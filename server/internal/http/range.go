...
func (h *HTTPRangeSpec) GetOffsetLength(resourceSize int64) (start, length int64, err error) {
	...
	//如果 start + length > obj.Size，说明 end > size，需要  修正
	end := start + length - 1
	if end > resourceSize {
		end = resourceSize - 1
		length = end - start + 1
...
// Parse an HTTP range header value into a HTTPRangeSpec
func ParseRequestRangeSpec(rangeString string) (hrange *HTTPRangeSpec, err error) {
	...
	offsetBegin := int64(-1)
	// Convert offsetBeginString only if it's not empty.
	...
	offsetEnd := int64(-1)
	// Convert offsetEndString only if it's not empty.
	...
	case offsetBegin > -1 && offsetEnd > -1:
		if offsetBegin > offsetEnd {
			return nil, ToError(ErrInvalidRange)
		}
	...
	default:
		// rangeString contains first and last byte positions missing. e.g. "bytes=-"
...
func (h *HTTPRangeSpec) ToHeader() (string, error) {
	...
	case h.Start >= 0 && h.End >= 0:
		if h.Start > h.End {
			return "", ToError(ErrInvalidRange)
		}
	case h.IsSuffixLength:
